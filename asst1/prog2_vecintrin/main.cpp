...
void clampedExpVector(float* values, int* exponents, float* output, int N) {
  ...
  float clampAt = 9.999999f;

  __cs149_vec_float x, result, maskClamp = _cs149_vset_float(9.999999f);
  __cs149_vec_int y, zero = _cs149_vset_int(0), ones = _cs149_vset_int(1);

  __cs149_mask maskAll, maskPositive, maskGtClamp, maskIsZero, maskInRange;

  for (int i=0; i < N; i+=VECTOR_WIDTH) {
    result = _cs149_vset_float(1.f);

    if (i + VECTOR_WIDTH >= N) {
      maskInRange = _cs149_init_ones(N - i);
    } else {
      maskInRange = _cs149_init_ones();
    }
    ...
    _cs149_vgt_int(maskPositive, y, zero, maskAll);
    maskIsZero = _cs149_mask_not(maskPositive);
    
    while (_cs149_cntbits(maskPositive)) {
      _cs149_vmult_float(result, result, x, maskPositive);
      _cs149_vsub_int(y, y, ones, maskPositive);
      _cs149_vgt_int(maskPositive, y, zero, maskPositive);
    }

    // only works for nonnegative exponents
    _cs149_vset_float(result, 1.f, maskIsZero);

    _cs149_vgt_float(maskGtClamp, result, maskClamp, maskAll);
    _cs149_vset_float(result, clampAt, maskGtClamp);
    _cs149_vstore_float(output + i, result, maskInRange);
  }
}
...
float arraySumVector(float* values, int N) {
  ...
  //
  float* partial_sums = new float[N / VECTOR_WIDTH];
